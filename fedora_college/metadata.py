#!/usr/bin/env python
# -*- coding: utf-8 -*-

""":mod:`Fedora_College.metadata` --- Project metadata

Information describing the project.
"""

# The package name, which is also the "UNIX name" for the project.
package = 'fedora_college'
project = "Fedora College"
project_no_spaces = project.replace(' ', '')
version = '0.1'
description = 'A virtual learning environment for Fedora'
authors = ['Eduardo Echeverria', 'Yohan Graterol']
authors_string = ', '.join(authors)
emails = ['echevemaster@fedoraproject.org', 'yograterol@fedoraproject.org']
license = 'BSD'
copyright = '2013 ' + authors_string
url = 'https://github.com/echevemaster/fedora-college'
